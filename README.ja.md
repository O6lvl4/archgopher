...
枠は見出しをつかむと中のカードごと動き、選択中は四隅で大きさを変えられ、Delete で消せます（中のカードは残ります）。見出しをクリックすると AZ 数と読み値が出ます。
キャンバスは自動で整形します。ノード・辺・枠の追加と削除、枠へのカードの出し入れのたびに、左から右へ並べ直し、枠は中のカードを囲む大きさに合わせます。数値の編集やカードを少し動かしただけでは並べ直しません。
