...
| アカウント単位の料金 | 段階料金・無料枠・地域ごとの固定料金は、単価の台帳に `pool`（`account` か `region`）・`tiered`（行のキーが段の始まり）・`free`（毎月の無料の量）・`freeGroup`（複数の単価で1つの無料枠を分け合う。Lambda の x86 と Arm、CloudFront の全価格帯）・`combine: max`（何本あっても1回払う料金）で書く。エンジンは全ノードを読んだあと、同じ単価を読んだ行を合算して1回だけ段階と無料枠を当て、量の比で各行に配る。宣言は1つのアカウントとみなす。`billing: {free: false}` で無料枠を使わない | 資源ごとに段階や無料枠を当てると、同じ種類の資源が多いほど高く出る。組織で無料枠を使い切っているアカウントは無料枠を外せる。1ノードだけ読むとき（ケース・gaps）は、そのノードだけのアカウントとして計算する |
