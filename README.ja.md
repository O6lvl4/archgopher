...
枠の見出しをクリックすると AZ 数と読み値が出て、Delete で枠を消せます（中のカードは残ります）。
キャンバスは常に自動で整形します。位置は保存せず、読み込み・追加・削除・カードを落とすたびに左から右へ並べ直し、枠は中のカードを囲む大きさに合わせます。カードのドラッグは、どの枠に入れるかを伝えるためだけに使います。
