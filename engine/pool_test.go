...
// that share volume tiers and free units across the account.
...
func poolBooks(t *testing.T) book.Books {
	...
	prices, err := book.Book{
		"fee.requests": {Unit: "request", Per: 1e6, Source: "test", Pool: book.PoolRegion, Free: 1e6, Tiered: true, Verified: true, Rows: map[string]map[string]*float64{
...
func poolSpec(free *bool) model.Spec {
	return model.Spec{Region: "r1", Billing: model.Billing{Free: free},
		Nodes: []model.Node{
...
func TestPoolsBillTheAccountOnce(t *testing.T) {
	...
	reg.Register(fee)
	res, err := Run(poolSpec(nil), reg, poolBooks(t))
	if err != nil {
		t.Fatal(err)
	}
	// 15M requests: 1M free, 9M × $10/M, 5M × $5/M = $115, shared 6:9.
...
func TestPoolsWithoutFreeUnits(t *testing.T) {
	reg := scouter.Registry{}
	reg.Register(fee)
	no := false
	res, err := Run(poolSpec(&no), reg, poolBooks(t))
	if err != nil {
		t.Fatal(err)
	}
	// 10M × $10/M + 5M × $5/M
	if got := *res.Pools[1].MonthlyUSD; math.Abs(got-125) > 1e-9 {
		t.Errorf("requests pool = %v, want 125", got)
	}
}
