...
| アカウント単位の料金 | 段階料金・プランに含まれる量・地域ごとの固定料金は、単価の台帳に `pool`（`account` か `region`）・`tiered`（行のキーが段の始まり）・`included`（有料プランが毎月含む量）・`combine: max`（何本あっても1回払う料金）で書く。エンジンは全ノードを読んだあと、同じ単価を読んだ行を合算して1回だけ段階を当て、量の比で各行に配る。宣言は1つのアカウントとみなす。無料枠（AWS の Always Free、Azure の無料付与、Google Cloud の無料枠）は数えない。価格表が0円の最初の段として持つ無料付与は、最初の有料の段の単価で数える | 資源ごとに段階を当てると、同じ種類の資源が多いほど高く出る。無料枠は「毎月いくらかかるか」ではなく、統合請求では組織全体で1回しか効かないので、数えるとかえって誤る。1ノードだけ読むとき（ケース・gaps）は、そのノードだけのアカウントとして計算する |
| アカウントの上限値 | 上限の既定値は公開値でカタログに持つ。アカウントに適用中の値は `archgopher quotas` が AWS Service Quotas から読み、宣言の `quotas:` に書く（レポートでは `(account)`）。認証は AWS CLI の一時鍵を借り、SigV4 は自前で署名して SDK に依存しない | 引き上げ済みの上限や新しいアカウントの低い初期値は公開値と違う。アカウント固有の値は利用者のものなのでカタログ（公開）には入れず、その宣言の中だけに置く |
...
| Cloudflare | 単価を取り込む API が無いので、料金ページから手で読み、確かめた日付（checkedAt）を付けて確認済みにする。`sync` では見直せない。単価は全地域で同じなので `*` の1行で持ち、どの地域の宣言でも同じ額になる。Workers 有料プランに含まれる量（Workers のリクエストと CPU 時間・KV・Durable Objects・D1・Queues）は、アカウント全体で1回だけ差し引く（`included`）。月額 $5 は何ノード置いても1回。R2 の無料枠と Workers AI のニューロン（1日1万）は無料枠なので差し引かない | 含まれる量は月額で買っている分なので料金の一部として数え、アカウント全体で共有されるので資源ごとには引かない。無料枠は数えない方針に合わせる。Cloudflare だけの宣言では地域が無くても警告しない |
| ConoHa VPS | GMO の `gmo-internet/conohavps`（ベータ）のサーバーとボリュームを読む。キーペア・セキュリティグループとそのルールは無料。単価は料金ページから手で読み、Cloudflare と同じく checkedAt を付けて確認済みにし、どの地域の宣言でも同じ額にする。サーバーはフレーバーの UUID からプラン名を引き、時間課金の月額上限で数える。まとめトク・Windows・DB・GPU のプランは未対応と出す。料金は税込みの円のまま台帳に書き（`currency: JPY`・`tax: 0.1`）、読み込み時に税を抜き、台帳の為替 `fx.jpy`（1円あたりのドル、確かめた日付つき）でドルにする | ConoHa は停止中も削除するまで課金され、月額上限を超えないので、宣言に置いた資源の定常の額は月額上限そのもの。単価を料金ページと同じ数字で持てば照合できる。税込みのままだと他社の税抜き単価と比べられないので税を抜く。為替は動くので1か所の値にし、変えれば全体が追随する |
