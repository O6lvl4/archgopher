...
| `aws_rds_cluster` | Storage, Aurora Serverless v2 ACU-hours (provisioned instances are their own nodes), storage, I/O (Standard or I/O-Optimized), the managed master password secret | Peak ACU against max capacity |
...
| `aws_redshift_cluster` | Node-hours, managed storage, concurrency scaling, Spectrum, backups in three tiers | Stored data against the nodes' storage |
