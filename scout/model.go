// Package scout is the engine of arch-scouter: it pushes load from entry
// nodes through a graph of resources and reads each node on four
// dimensions (monthly cost, headroom against limits, latency, availability).
//
// The package has no I/O and no knowledge of any cloud provider. Providers
// contribute Scouters (one per resource type) and reference books (prices,
// quotas, SLAs).
package scout
