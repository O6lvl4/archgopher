...
// ScheduleLoad turns rate() and cron() expressions into a load. The peak rate
// is one fire per interval for rate(), and the average rate for cron().
func ScheduleLoad(expr string) (scout.Load, error) {
	expr = strings.TrimSpace(expr)
	switch {
	case strings.HasPrefix(expr, "rate(") && strings.HasSuffix(expr, ")"):
		f := strings.Fields(expr[5 : len(expr)-1])
		if len(f) != 2 {
			return scout.Load{}, fmt.Errorf("cannot read %q", expr)
		}
		...
		if unit == 0 {
			return scout.Load{}, fmt.Errorf("unknown unit in %q", expr)
		}
		per := 1 / (n * unit)
		return scout.Load{Monthly: per * scout.SecondsPerMonth, PeakPerSecond: per}, nil
	case strings.HasPrefix(expr, "cron(") && strings.HasSuffix(expr, ")"):
		f := strings.Fields(expr[5 : len(expr)-1])
		if len(f) != 6 {
			return scout.Load{}, fmt.Errorf("cannot read %q: want 6 fields", expr)
		}
		...
		return scout.Load{Monthly: monthly, PeakPerSecond: monthly / scout.SecondsPerMonth}, nil
	case expr == "":
		return scout.Load{}, fmt.Errorf("no schedule expression")
	}
	return scout.Load{}, fmt.Errorf("cannot read schedule %q", expr)
}
